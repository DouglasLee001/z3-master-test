...
struct variable{
    ...
    std::vector<uint64_t>    neighbor_var_idxs_idl;
    //bool vars in same clause
    std::vector<uint64_t>    neighbor_var_idxs_bool;
    //vars in same lit
    std::vector<uint64_t>    neighbor_var_idxs_in_lit;
...
public:
    ...
    std::vector<distinct_var>  distinct_vars;//the value for variable to choose
    size_t                  transfer_sym_to_distinct_var(size_t sym,size_t size);//to build the distinct vars std::vector, size means that of the optional values
    std::map<size_t,size_t>     sym2distinct_var;
    int                    transfer_to_dimacs(int num);//transfer (value*|vars|+var_idx) to a num
    std::map<int,int>            distinct2dimacs;
    std::vector<int>             distinct_num;//save (value*|vars|+var_idx)
    ...
    std::vector<int64_t> exist_time;


    //data structure
//    std::vector<uint64_t> _unsat_clauses;
    std::vector<uint64_t> _unsat_soft_clauses;
    std::vector<uint64_t> _unsat_hard_clauses;
//    std::vector<uint64_t> _index_in_unsat_clauses;
    ...
    std::vector<int>     restart_unsat_num_in_unsat_clause;
    //resolution
    void            resolution();
